...
class TokenService {
    async generateTokenPair(payload) {
        try {
            const accessToken = jsonwebtoken_1.default.sign(payload, config_1.config.SECRET_ACCESS_KEY, { expiresIn: config_1.config.EXPIRES_IN_ACCESS }); // first - what to hash, second - key word
    ...
    async saveToken(userId, refreshToken) {
        try {
            ...
            if (tokenFromDb) {
                ...
                return tokenRepository_1.tokenRepository.createToken(tokenFromDb); // refreshed token
            }
            return tokenRepository_1.tokenRepository.createToken({ refreshToken, userId }); // created token
        }
        catch (e) {
            console.log(e);
            throw new Error(e);
