...
class UserService {
  public async getUsersBySpecificText() {
    return userRepository.getUsersByPredefinedText();
  }
  ...
  public async updateUserEmailAndPasswordById(
    userId: string,
    password: string,
    email: string
  ) {
    return userRepository.updateUserById(userId, password, email);
  }
